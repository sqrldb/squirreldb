...
mod integration {
  ...
  async fn test_mcp_crud_operations() {
    ...
    // Create
    let doc = backend
      .insert("items", json!({"name": "Widget", "price": 9.99}))
      .await
      .unwrap();
    let id = doc.id;

    // Read
    let retrieved = backend.get("items", id).await.unwrap().unwrap();
    assert_eq!(retrieved.data["name"], "Widget");

    // Update
    let updated = backend
      .update("items", id, json!({"name": "Super Widget", "price": 19.99}))
      .await
      .unwrap()
      .unwrap();
    ...
    // Delete
    let deleted = backend.delete("items", id).await.unwrap().unwrap();
    assert_eq!(deleted.id, id);

    // Verify deleted
    let gone = backend.get("items", id).await.unwrap();
    assert!(gone.is_none());
  }
  ...
  async fn test_mcp_query_with_filter() {
    ...
    // Insert test data
    backend
      .insert("users", json!({"name": "Alice", "age": 25}))
      .await
      .unwrap();
    backend
      .insert("users", json!({"name": "Bob", "age": 30}))
      .await
      .unwrap();
    backend
  ...
  async fn test_mcp_update_nonexistent() {
    ...
    let result = backend
      .update("users", fake_id, json!({"name": "Ghost"}))
      .await
      .unwrap();
  ...
  async fn test_mcp_multiple_collections_isolation() {
    ...
    backend
      .insert("collection_a", json!({"value": 1}))
      .await
      .unwrap();
    backend
      .insert("collection_a", json!({"value": 2}))
      .await
      .unwrap();
    backend
      .insert("collection_b", json!({"value": 100}))
      .await
      .unwrap();
  ...
  async fn test_mcp_large_document() {
    ...
    let doc = backend
      .insert("large", serde_json::Value::Object(large_data))
      .await
      .unwrap();

    let retrieved = backend.get("large", doc.id).await.unwrap().unwrap();
  ...
  async fn test_mcp_special_characters_in_data() {
    ...
    let retrieved = backend.get("special", doc.id).await.unwrap().unwrap();
  ...
  async fn test_mcp_numeric_types() {
    ...
    let retrieved = backend.get("numbers", doc.id).await.unwrap().unwrap();
  ...
  async fn test_mcp_boolean_and_null() {
    ...
    let retrieved = backend.get("booleans", doc.id).await.unwrap().unwrap();
  ...
  async fn test_mcp_deeply_nested() {
    ...
    let retrieved = backend.get("nested", doc.id).await.unwrap().unwrap();
    assert_eq!(
  ...
  async fn test_mcp_array_operations() {
    ...
    let retrieved = backend.get("arrays", doc.id).await.unwrap().unwrap();
...
mod query_engine {
  ...
  async fn test_query_basic_table() {
    ...
    backend.insert("test", json!({"x": 1})).await.unwrap();
    backend.insert("test", json!({"x": 2})).await.unwrap();
  ...
  async fn test_query_filter_equality() {
    ...
    backend
      ...
      .unwrap();
    backend
      .insert("users", json!({"name": "Charlie", "role": "admin"}))
      .await
      .unwrap();
  ...
  async fn test_query_filter_comparison() {
    ...
    backend
      .insert("products", json!({"name": "A", "price": 10}))
      .await
      .unwrap();
    backend
      .insert("products", json!({"name": "B", "price": 25}))
      .await
      .unwrap();
    backend
      .insert("products", json!({"name": "C", "price": 50}))
      .await
      .unwrap();
  ...
  async fn test_query_empty_result() {
    ...
    backend.insert("data", json!({"value": 1})).await.unwrap();
  ...
  async fn test_query_map_transform() {
    ...
    backend
      .insert("users", json!({"firstName": "Alice", "lastName": "Smith"}))
      .await
      .unwrap();
