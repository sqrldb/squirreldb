...
fn test_document_deserialization() {
  let id = Uuid::new_v4();
  let json = format!(
    r#"{{"id":"{}","project_id":"00000000-0000-0000-0000-000000000000","collection":"users","data":{{"name":"Alice"}},"created_at":"2024-01-15T10:30:00Z","updated_at":"2024-01-15T10:30:00Z"}}"#,
    id
  );
...
fn test_query_spec_with_all_fields() {
  let spec = QuerySpec {
    project_id: Some(DEFAULT_PROJECT_ID),
    table: "users".into(),
