...
async fn test_insert_to_different_collections() {
  ...
  backend
    .insert(DEFAULT_PROJECT_ID, "users", json!({"type": "user"}))
    .await
    .unwrap();
  backend
    .insert(DEFAULT_PROJECT_ID, "posts", json!({"type": "post"}))
    .await
    .unwrap();
  backend
    .insert(DEFAULT_PROJECT_ID, "comments", json!({"type": "comment"}))
    .await
    .unwrap();
...
async fn test_list_with_limit() {
  ...
  let docs = backend
    .list(DEFAULT_PROJECT_ID, "items", None, None, Some(5), None)
    .await
    .unwrap();
  assert_eq!(docs.len(), 5);
}
...
async fn test_list_limit_larger_than_count() {
  ...
  let docs = backend
    .list(DEFAULT_PROJECT_ID, "items", None, None, Some(100), None)
    .await
    .unwrap();
...
async fn test_list_with_filter() {
  ...
  let docs = backend
    .list(DEFAULT_PROJECT_ID, "users", Some(filter), None, None, None)
    .await
    .unwrap();
...
async fn test_invalid_collection_names_rejected() {
  ...
  for name in &invalid_names {
    let result = backend.insert(DEFAULT_PROJECT_ID, name, json!({})).await;
    assert!(
      result.is_err(),
...
async fn test_concurrent_read_write() {
  ...
  let doc = backend
    .insert(DEFAULT_PROJECT_ID, "users", json!({"counter": 0}))
    .await
    .unwrap();
