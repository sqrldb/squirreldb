...
pub struct CachingSection {
  /// Port for Redis-compatible protocol (default: 6379)
  ...
  /// Eviction policy: lru, lfu, random, noeviction
  #[serde(default = "default_cache_eviction")]
  pub eviction: String,
  ...
  #[serde(default)]
  pub snapshot: CacheSnapshotSection,
}

/// Cache snapshot persistence configuration
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct CacheSnapshotSection {
  /// Enable periodic snapshots
  #[serde(default)]
  pub enabled: bool,

  /// Path to snapshot file
  #[serde(default = "default_cache_snapshot_path")]
  pub path: String,
...
fn default_cache_eviction() -> String {
  "lru".to_string()
}

fn default_cache_snapshot_path() -> String {
...
impl ServerConfig {
  ...
  pub fn cache_address(&self) -> String {
    format!("{}:{}", self.server.host, self.caching.port)
  }
}
